...
public class DW_PoolGUI : DW_DemoGUI {
    ...
    override protected void Start() {
        ...
        _sceneName = DW_SceneManagerWrapper.LoadedLevelName;
    ...
    private void OnGUI() {
        ...
        if (BuoyantCrate != null) {
            ...
            if (DW_GUILayout.Button("Drop a box!", 180f)) {
                ...
                Instantiate(
                    BuoyantCrate, 
                    new Vector3(Random.Range(bounds.min.x, bounds.max.x), 10f, Random.Range(bounds.min.z, bounds.max.z)),
        ...
        DW_GUILayout.tooltip = 
            string.Format(
            "Simulation quality. Larger values result in more smooth visuals.\n\nGrid:{0}x{1}, {2} vertices, {3} triangles.",
            Water.GridSize.x, 
            Water.GridSize.y, 
            Water.GridSize.x * Water.GridSize.y, 
            Water.GridSize.x * Water.GridSize.y * 2
        );
