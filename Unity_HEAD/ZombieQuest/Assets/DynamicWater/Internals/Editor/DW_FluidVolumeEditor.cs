...
public class DW_FluidVolumeEditor : UndoEditor<FluidVolume> {
    ...
    protected override void OnInspectorGUIDraw() {
        ...
        _object.Depth =
            Mathf.Clamp(
                EditorGUILayout.FloatField(
                    new GUIContent(
                        _object.GetType() == typeof(FluidVolume) ? "Height" : "Depth",
        ...
        _object.Density =
            Mathf.Clamp(
                EditorGUILayout.FloatField(
                    new GUIContent("Density", "Fluid density in kg/m^3"),
                    _object.Density),
                0f,
                10000f
            );
    ...
    protected override void OnSceneGUIDraw() {
        ...
        const float sizeChangeThreshold = Vector3.kEpsilon * 10f;

        // Apply Size
        if (!float.IsPositiveInfinity(sizeNew.x) && !float.IsPositiveInfinity(sizeNew.y) && (sizeOld - sizeNew).magnitude >= sizeChangeThreshold) {
    ...
    protected override void OnEnable() {
        ...
        EditorUtility.SetSelectedRenderState(_object.GetComponent<Renderer>(), EditorSelectedRenderState.Hidden);
#else
        EditorUtility.SetSelectedWireframeHidden(_object.GetComponent<Renderer>(), true);
#endif
    }
}
